...
pub type block_no = cty::c_uint;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct block {
    pub bytes: [cty::c_char; 512],
}
...
pub struct treedisk_state {
    ...
    pub below_ino: cty::c_uint,
    pub ninodes: cty::c_uint,
}
#[derive(Copy, Clone)]
#[repr(C)]
...
static mut log_rpb: cty::c_uint = 0;
static mut null_block: block_t = block_t { bytes: [0; 512] };
unsafe extern "C" fn panic(mut s: *const cty::c_char) {
    ((*earth).tty_fatal).expect("non-null function pointer")(s);
}
unsafe extern "C" fn log_shift_r(mut x: block_no, mut nbits: cty::c_uint) -> block_no {
    if nbits as cty::c_ulong
        >= (::std::mem::size_of::<block_no>() as cty::c_ulong)
            .wrapping_mul(8 as cty::c_int as cty::c_ulong)
    {
        return 0 as cty::c_int as block_no;
    }
    return x >> nbits;
}
...
unsafe extern "C" fn treedisk_getsize(
    ...
    if treedisk_get_snapshot(&mut snapshot, ts, ino) < 0 as cty::c_int {
        return -(1 as cty::c_int);
    }
    return (*snapshot.inode).nblocks as cty::c_int;
}
...
unsafe extern "C" fn treedisk_read(
    ...
    loop {
        ...
        if nlevels == 0 as cty::c_int as cty::c_uint {
            return 0 as cty::c_int;
        }
...
unsafe extern "C" fn treedisk_write(
    ...
    let mut ts: *mut treedisk_state = (*this_bs).state as *mut treedisk_state;
    let mut dirty_inode: cty::c_int = 0 as cty::c_int;
    ...
    if treedisk_get_snapshot(snapshot, ts, ino) < 0 as cty::c_int {
        return -(1 as cty::c_int);
    }
    let mut nlevels: cty::c_uint = 0 as cty::c_int as cty::c_uint;
    if (*(*snapshot).inode).nblocks > 0 as cty::c_int as cty::c_uint {
    ...
    if (*(*snapshot).inode).nblocks == 0 as cty::c_int as cty::c_uint {
        ...
        while nlevels_after > nlevels {
            ...
            tib.refs[0 as cty::c_int as usize] = (*(*snapshot).inode).root;
    ...
    if dirty_inode != 0 {
        ...
        {
            panic(b"treedisk_write: inode block\0" as *const u8 as *const cty::c_char);
        }
    }
    let mut b: block_no = 0;
    ...
    loop {
        ...
        if b == 0 as cty::c_int as cty::c_uint {
            *parent_no = treedisk_alloc_block(ts, snapshot);
            ...
            memset(
                &mut tib_0 as *mut treedisk_indirblock as *mut cty::c_void,
                0 as cty::c_int,
                512 as cty::c_int as cty::c_ulong,
            );
        } else {
            ...
            {
                panic(b"treedisk_write\0" as *const u8 as *const cty::c_char);
            }
        }
        nlevels = nlevels.wrapping_sub(1);
        let mut index: cty::c_uint =
    ...
    if (Some(((*(*ts).below).write).expect("non-null function pointer")))
        ...
        < 0 as cty::c_int
    {
        panic(b"treedisk_write: data block\0" as *const u8 as *const cty::c_char);
    }
    return 0 as cty::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn treedisk_init(
    ...
    if log_rpb == 0 as cty::c_int as cty::c_uint {
        loop {
...
pub unsafe extern "C" fn setup_freelist(
    ...
    let mut freelist_block: block_no = 0 as cty::c_int as block_no;
    let mut i: cty::c_uint = 0;
    while next_free < nblocks {
        freelist_data[0 as cty::c_int as usize] = freelist_block;
        let fresh7 = next_free;
        next_free = next_free.wrapping_add(1);
        freelist_block = fresh7;
        i = 1 as cty::c_int as cty::c_uint;
        ...
        while (i as cty::c_ulong)
            < (512 as cty::c_int as cty::c_ulong)
                .wrapping_div(::std::mem::size_of::<block_no>() as cty::c_ulong)
        {
            freelist_data[i as usize] = 0 as cty::c_int as block_no;
            i = i.wrapping_add(1);
        }
        ...
        {
            panic(b"treedisk_setup_freelist\0" as *const u8 as *const cty::c_char);
        }
    }
...
pub unsafe extern "C" fn treedisk_create(
    ...
    if superblock.superblock.n_inodeblocks == 0 as cty::c_int as cty::c_uint {
        let mut superblock_0: treedisk_block = treedisk_block {
            datablock: block_t { bytes: [0; 512] },
        };
        memset(
            &mut superblock_0 as *mut treedisk_block as *mut cty::c_void,
            0 as cty::c_int,
            512 as cty::c_int as cty::c_ulong,
        );
